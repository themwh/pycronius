...
class TestBasicCronRule(unittest.TestCase):
    ...
    def test_parse_field(self):
        strs = {
            ...
            "2-9": range(2, 10),
            "20-30": range(20, 31),
            "20-30/2": range(20, 31, 2),
            "*": range(0, 60),
            "*/3": range(0, 60, 3),
            "1,2-10/2,13-15": [1, 2, 4, 6, 8, 10, 13, 14, 15]
        }
        for f, v in list(strs.items()):
    ...
    def test_parse(self):
        ...
        self.assertTrue( all([m in rule.rulesets["minutes"] for m in range(0, 60)]) )
        self.assertTrue( all([h in rule.rulesets["hours"] for h in range(7, 20)]) )
        self.assertTrue( all([d in rule.rulesets["dom"] for d in range(1, 32)]) )
        self.assertTrue( all([m in rule.rulesets["month"] for m in range(1, 13)]) )
        self.assertTrue( all([d in rule.rulesets["dow"] for d in range(1, 6)]) )
        self.assertTrue( all([y in rule.rulesets["year"] for y in range(1999, 2021, 2)]) )
        self.assertFalse( all([y in rule.rulesets["year"] for y in range(1999, 2021)]) )
        self.assertFalse( all([y in rule.rulesets["year"] for y in range(1999, 2024, 2)]) )
...
class TestCronRangeRule(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        self.assertTrue( all([m in rule.rulesets["month"] for m in range(1, 13)]) )
        self.assertTrue( all([d in rule.rulesets["dow"] for d in range(1, 6)]) )
        self.assertTrue( all([y in rule.rulesets["year"] for y in range(2000, 2021)]) )
